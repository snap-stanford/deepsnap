...
class Batch(Graph):
    ...
    @staticmethod
    def from_data_list(data_list: List[Graph], follow_batch: List = None,
        ...
        if transform is not None:
            data_list = [data.apply_transform(
                            transform,
                            deep_copy=True,
                            **kwargs)
    ...
    def _reconstruct_dict(
        ...
        for key in keys:
            if isinstance(batched_dict[key], dict):
                ...
                data_dict[key] = {}
                inner_keys = [inner_key for inner_key in batched_dict[key].keys()
                              if inner_key[-5:] != 'batch']
                inner_cumsum = {inner_key: 0 for inner_key in inner_keys}
                inner_slices = slices[key]
                self._reconstruct_dict(graph_idx, inner_keys,
                                       data_dict[key], inner_cumsum,
                                       inner_slices,
                                       batched_dict[key], graph)
                continue
            ...
            if torch.is_tensor(batched_dict[key]):
                ...
                data_dict[key] = batched_dict[key][
