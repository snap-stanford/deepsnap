...
class Graph(object):
    ...
    def _update_attributes(self):
        ...
        if (self.G.number_of_nodes() == 0):
            raise ValueError('in _update_attributes, number of nodes in Graph '
                             'G must be larger than 0')
        if (self.G.number_of_edges() == 0):
            raise ValueError('in _update_attributes, number of edges in Graph '
                             'G must be larger than 0')
    ...
    def _edge_to_index(self, edges):
        ...
        """
        if (len(edges) == 0):
            raise ValueError('in _edge_to_index, len(edges) must be '
                             'larger than 0')
        ...
        if self.is_undirected():
            edge_index = torch.cat(
                [edge_index, torch.flip(edge_index, [1])], dim=0)
    ...
    def _get_edge_attributes_by_key(self, edges, key: str):
        ...
        if (len(edges) == 0):
            raise ValueError('in _get_edge_attributes_by_key, '
    ...
    def _split_edge(self, split_ratio: float):
        ...
        if (self.num_edges < len(split_ratio)):
            raise ValueError('in _split_node num of edges are smaller than'
                             'number of splitted parts')
        ...
        for i, split_ratio_i in enumerate(split_ratio):
            if i != len(split_ratio) - 1:
                num_split_i = \
                    1 + int(split_ratio_i * (self.num_edges - len(split_ratio)))
    ...
    def split_link_pred(self, split_ratio: Union[float, List[float]]):
        ...
        if len(split_ratio) == 2:
            if (self.num_edges < 2):
                raise ValueError('in split_link_pred num of edges are'
        ...
        if len(split_ratio) == 3:
            graph_test = Graph(self._edge_subgraph_with_isonodes(
    ...
    def _create_neg_sampling(self, negative_sampling_ratio: float, resample: bool = False):
        r"""
        ...
        if resample and self._num_positive_examples is not None:
            ...
            self.edge_label_index = self.edge_label_index[:, :self._num_positive_examples]
        ...
        if (len(edge_index_all) > 0):
            negative_edges = \
                self.negative_sampling(edge_index_all, self.num_nodes, num_neg_edges)
        else:
        ...
        self.edge_label_index = torch.cat(
            (self.edge_label_index, negative_edges), -1)
        self.edge_label = torch.cat((positive_label, negative_label), -1).type(torch.long)
    ...
    def pyg_to_graph(data, verbose: bool = False, fixed_split: bool = False):
        ...
        kwargs['edge_feature'] = data.edge_attr if 'edge_attr' in data.keys \
            else None
        kwargs['node_label'], kwargs['edge_label'] = None, None
        kwargs['graph_feature'], kwargs['graph_label'] = None, None
        if kwargs['node_feature'] is not None and \
                data.y.size(0) == kwargs['node_feature'].size(0):
            kwargs['node_label'] = data.y
        elif kwargs['edge_feature'] is not None and \
                data.y.size(0) == kwargs['edge_feature'].size(0):
